...
/*********************************************************************************************************************/
BiasedPriorInfo::BiasedPriorInfo(const rimesa::RobotId& other_robot, const BiasedPriorType& type,
                                 const gtsam::Value& init_estimate, double init_penalty) {
  this->other_robot = other_robot;
...
gtsam::NonlinearFactor::shared_ptr factory(
    ...
    const std::optional<GraduatedBiasedPriorParams>& kernel_params) {
  // Construct the kernel if parameters are provided
  risam::SIGKernel::shared_ptr kernel;
  if (kernel_params) {
    kernel = boost::make_shared<risam::SIGKernel>(
        risam::SIGKernel::shapeParamFromChi2(vals_ptr->shared_estimate->dim(), kernel_params->chi2_threshold),
        kernel_params->mu_update_strat, kernel_params->mu_init_inc_strat);
  }

  if (type == BiasedPriorType::GEODESIC) {
    if (kernel_params) {
      return graduated_factory<GeodesicBiasedPrior>(kernel, key, vals_ptr, biased_prior_noise_model_sigmas);
    } else {
      return factory<GeodesicBiasedPrior>(key, vals_ptr, biased_prior_noise_model_sigmas);
    }
  } else if (type == BiasedPriorType::SPLIT) {
    if (kernel_params) {
      return graduated_factory<SplitBiasedPrior>(kernel, key, vals_ptr, biased_prior_noise_model_sigmas);
    } else {
