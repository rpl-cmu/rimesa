...
gtsam::NonlinearFactor::shared_ptr factory(
  ...
  if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector>)) {
    ...
    return boost::make_shared<GeodesicBiasedPrior<gtsam::Vector>>(key, vals_ptr, nm);
  }
  /** POINT2 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector2>)) {
    // Always use geodesic for linear variables
    return boost::make_shared<GeodesicBiasedPrior<gtsam::Point2>>(key, vals_ptr, nm);
  }
  /** POINT3 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector3>)) {
    // Always use geodesic for linear variables
    return boost::make_shared<GeodesicBiasedPrior<gtsam::Point3>>(key, vals_ptr, nm);
  }
  /** POSE2 **/
...
gtsam::NonlinearFactor::shared_ptr graduated_factory(
    const risam::SIGKernel::shared_ptr& kernel, const gtsam::Key& key, const BiasedPriorInfo::shared_ptr& vals_ptr,
    const std::optional<std::pair<double, double>>& biased_prior_noise_model_sigmas) {
  // Setup the Base noise model
  gtsam::SharedNoiseModel nm = gtsam::noiseModel::Unit::Create(vals_ptr->shared_estimate->dim());
  ...
  if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector>)) {
    // Always use geodesic for linear variables
    return risam::make_shared_graduated<GeodesicBiasedPrior<gtsam::Vector>>(kernel, key, vals_ptr, nm);
  }
  /** POINT2 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector2>)) {
    // Always use geodesic for linear variables
    return risam::make_shared_graduated<GeodesicBiasedPrior<gtsam::Point2>>(kernel, key, vals_ptr, nm);
  }
  /** POINT3 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Vector3>)) {
    // Always use geodesic for linear variables
    return risam::make_shared_graduated<GeodesicBiasedPrior<gtsam::Point3>>(kernel, key, vals_ptr, nm);
  }
  /** POSE2 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Pose2>)) {
    if (biased_prior_noise_model_sigmas) {
    ...
    return risam::make_shared_graduated<BIASED_PRIOR<gtsam::Pose2>>(kernel, key, vals_ptr, nm);
  }
  /** POSE3 **/
  else if (typeid(*vals_ptr->shared_estimate) == typeid(gtsam::GenericValue<gtsam::Pose3>)) {
    if (biased_prior_noise_model_sigmas) {
    ...
    return risam::make_shared_graduated<BIASED_PRIOR<gtsam::Pose3>>(kernel, key, vals_ptr, nm);
  } else {
    throw std::runtime_error("Invalid shared_estimate type passed in vals_ptr to Biased Prior factory");
  }
}
